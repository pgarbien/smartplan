...
const NewLevelModal = ({
  ...
  const modalContent = (
    <Fragment>
      <div className="new-level-modal-content">
        ...
        <input
          ...
          id="level-name"
          name="level-name"
          placeholder={t("popups.defaultLevel")}
          onChange={(event) => {
        ...
        <br />
        <br />
        <label for="level-blueprint" className="upload-button">
          {levelBlueprint ? levelBlueprint.name : t("popups.uploadBlueprint")}
        </label>
        <input
          type="file"
          id="level-blueprint"
        ...
        <button
          ...
          onClick={() => {
            levelBlueprint
              ? createNewLevel()
              : addNewLevel(levelName, null, editedLevel);
          }}
          disabled={levelName == null || levelName === ""}
        >
          {editedLevel ? t("popups.update") : t("popups.create")}
        </button>
        {editedLevel ? (
          <button
  ...
  const createNewLevel = () => {
    ...
    mAxios
      .post("/file/upload", blueprintFile)
      .then((response) => {
  ...
  return (
    <Modal
      title={editedLevel ? t("popups.editLevel") : t("popups.addLevel")}
      onCloseModal={() => {
        setShowModal(!canClose);
      }}
      canClose={canClose}
    >
      {" "}

...
const Manager = ({
  ...
  const fetchDevices = () => {
    mAxios
      .get("/devices?levelId=" + location.levels[activeLevel].id)
  ...
  const manageDefaultDeviceAction = (device) => {
    mAxios
      .post(`/devices/actions/${device.id}`, {
        actionType: device.defaultAction,
      })
      .then(() => {
        setTimeout(
          () =>
  ...
  const setDeviceDetails = (device, deviceDetails) => {
    const deviceVisualState = deviceDetails.possibleVisualStates.find(
      (state) => state == (deviceDetails.state.on ? "on" : "off")
    );
    device.deviceState = deviceDetails.state.connected
      ? deviceDetails.state.on
        ? DeviceState.ACTIVE
        : DeviceState.NOT_ACTIVE
      : DeviceState.DISABLED;
    device.activeIconId = deviceDetails.possibleVisualStates.indexOf(
      deviceVisualState
    );
    ...
    Object.keys(deviceDetails.state).map((field, index) => {
      if (
        deviceDetails.stateDetails &&
        deviceDetails.stateDetails[field]?.quickView
      ) {
        if (displayedState.length > 0) displayedState += ", ";
        displayedState +=
          deviceDetails.state[field] +
          "" +
          deviceDetails.stateDetails[field]?.unit;
      }
    });
    ...
    <Fragment>
      <div class="body-container manager-page">
        <div class="localization-header">
          <div class="left-header-wrapper">
            <h2>
              {t("managePage.manageDevsBeginning")}
              <span class="primary_color">
                {location ? location.name : "your"}
              </span>{" "}
              {t("managePage.manageDevsEnding")}
            </h2>
            <LevelsList
              location={location}
          ...
          <div class="button-header no-margin-top">
            <div
              ...
              onClick={() => {
                ...
                setBlueprintVisible(!blueprintVisible);
              }}
            >
              {t("popups.blueprint")}{" "}
            </div>
        ...
        <div className="manager-page-layout">
          ...
          <div className="right-container">
            ...
            <div className="right-container-buttons">
              <Link
                class="directional-button-link"
                to={location ? "/draw?locationId=" + location.id : "#"}
              >
              ...
              <Link
                class="directional-button-link"
                to={location ? "/draw/devices?locationId=" + location.id : "#"}
              >
                <div className="directional-button">
                  {t("tool.addDevices")} &nbsp;&gt;
                </div>
      ...
      {showManageDeviceModal ? (
        <ManageDeviceModal
          ...
          setShowModal={setShowManageDeviceModal}
          canClose={true}
        />
      ) : null}
    </Fragment>
